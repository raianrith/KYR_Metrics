...
import { buildStatusBreakdown, getTeamColor, type StatusBreakdownRow } from "@/lib/metrics";
import { STATUS_CHART_COLORS, statusColor, statusLabel, titleCase } from "@/lib/utils";
import {
  Bar,
  BarChart,
  CartesianGrid,
...
function StatusTooltip({
  ...
  return (
    <div className="bg-white border border-black/10 rounded-sm shadow-lg p-3 text-sm">
      ...
      <p className="text-xs font-body normal-case text-wg-suede mb-2">
        {d.onTrackPercent}% on track
      </p>
      <div className="space-y-1 text-xs font-body normal-case">
        <p className="text-green-700">Met/On Track: {d.met}</p>
        <p className="text-amber-700">At Risk: {d.atRisk}</p>
        <p className="text-red-700">Not Met: {d.notMet}</p>
      ...
      <div style={{ width: chartWidth, height }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 10, right: 10, left: -10, bottom: 56 }}>
            ...
            <Bar dataKey="met" stackId="a" fill={STATUS_CHART_COLORS.met} />
            <Bar dataKey="atRisk" stackId="a" fill={STATUS_CHART_COLORS.atRisk} />
            <Bar dataKey="notMet" stackId="a" fill={STATUS_CHART_COLORS.notMet} />
            <Bar dataKey="pending" stackId="a" fill={STATUS_CHART_COLORS.pending} radius={[2, 2, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
    </div>
  ...
  return (
    <div className="flex items-center justify-center gap-4 mt-4 text-[10px] text-wg-muted font-body normal-case flex-wrap">
      <span className="flex items-center gap-1.5">
        <span className="w-2.5 h-2.5 rounded-sm bg-green-600" /> Met
      </span>
      <span className="flex items-center gap-1.5">
        <span className="w-2.5 h-2.5 rounded-sm bg-wg-orange" /> At Risk
      </span>
      <span className="flex items-center gap-1.5">
        <span className="w-2.5 h-2.5 rounded-sm bg-red-600" /> Not Met
      </span>
      <span className="flex items-center gap-1.5">
...
export function TeamOverviewChart({ rows }: TeamOverviewChartProps) {
  ...
  return (
    <div>
      <div className="grid grid-cols-3 sm:grid-cols-4 xl:grid-cols-5 gap-x-4 gap-y-4 mb-6 pb-5 border-b border-black/5">
        {data.map((d) => (
          <div key={d.label} className="min-w-0">
            <p
              className="text-[10px] text-wg-muted truncate font-body normal-case tracking-normal"
              title={titleCase(d.label)}
            >
              {titleCase(d.label)}
            </p>
            <p className="font-display text-2xl text-wg-suede leading-none mt-1">
              {d.onTrackPercent}%
            </p>
          </div>
        ))}
      </div>
      <StatusBreakdownChart data={data} minBarWidth={88} />
    </div>
  );
}
...
export function StatusSummary({
  ...
  const items = [
    { label: "Met / On Track", value: met, color: "bg-green-600" },
    { label: "At Risk", value: atRisk, color: "bg-wg-orange" },
    { label: "Not Met", value: notMet, color: "bg-red-600" },
  ...
  return (
    <div className="space-y-4">
      <div>
        <p className="font-display text-5xl text-wg-suede leading-none">
          {onTrackPercent}%
        </p>
        <p className="text-xs text-wg-muted mt-2 font-body normal-case tracking-normal">
          On Track · {met} of {total} metrics
        </p>
      </div>
